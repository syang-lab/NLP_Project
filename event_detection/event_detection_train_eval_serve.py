...
def event_detection_train_eval(args):
    ...
    torch.save({'model_state_dict': model.state_dict()}, args.model_dir + "/event_domain_final.pt")
    return
...
def model_fn(model_dir):
    ...
    print("================  model init ===========================")
    pretrained_model=torch.load(model_path + "/event_domain_final.pt")
...
def predict_fn(tokenized_inputs, model):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    result=list()
    model.eval()
    for data in tokenized_inputs.items():
        ids = data['input_ids'].to(device)
        mask = data['attention_mask'].to(device)
