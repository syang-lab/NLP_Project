def summarization_train_eval(args):
    ...
    if not os.path.isdir(args.checkpoint_path + args.checkpoint_model):   
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained(args.checkpoint_model)
    else:
        logger.info(f" load pretrained model in pretrained model path: {args.checkpoint_path + args.checkpoint_model}")
        model = AutoModelForSeq2SeqLM.from_pretrained(args.checkpoint_path + args.checkpoint_model)
    
    
    # construct dataset
    training_eval_dataset = summarization_data(all_data[0:3000], tokenizer, domain_adaption=False, wwm_prob=0.1)
    ...
    train_dataset, eval_dataset = random_split(training_eval_dataset, [train_size, test_size], generator=torch.Generator().manual_seed(42))
    ...
    if not os.path.isfile(args.checkpoint_path + "/summarization_checkpoint.pt"):
        epoch_number = 0
    else:
        logger.info(f"load checkpoint model")
        checkpoint = torch.load(args.checkpoint_path + "/summarization_checkpoint.pt")
    ...
    for epoch in range(epoch_number, args.num_train_epochs):
        ...
        if check_early_stop.early_stop:
            ...
            model.save_pretrained(args.save_model + "/summarization_early_stop"+str(epoch))
            break
        ...
        if epoch % 5 == 0:
            ...
            }, args.checkpoint_path + "/summarization_checkpoint.pt")

    
    logger.info("save final model")
    model.save_pretrained(args.save_model + "/summarization_final")
    return
...
if __name__=="__main__":
    import os
    import sys
    sys.path.insert(0, os.path.abspath('./code_summary/'))
    ...
    parser.add_argument("--checkpoint_model",type=str,default="t5-small")
    ...
    # train
    summarization_train_eval(parser.parse_args())